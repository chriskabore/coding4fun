...
@WebMvcTest(ChallengeAttemptRestController.class)
@AutoConfigureJsonTesters
public class ChallengeAttemptRestControllerTest {
    ...
    @Autowired
    private JacksonTester<ChallengeAttempt> jsonResponseAttempt;
    ...
    void postValidResult() throws Exception{
        ...
        ChallengeAttempt expectedResponse = new ChallengeAttempt(attemptId,user.getUserId(), 50,
                70, 3500,true);
        ...
        then(response.getContentAsString()).isEqualTo(jsonResponseAttempt.write(expectedResponse).getJson());
    }

    @Test
    void postInvalidResult() throws Exception{
        // given an attempt with invalid input data
        ChallengeAttemptDTO attemptDTO = new ChallengeAttemptDTO(2000,
                -70, "john", 1);
        // when
        MockHttpServletResponse response = mockMvc.perform(
                post("/attempts").contentType(MediaType.APPLICATION_JSON)
